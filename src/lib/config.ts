  ...
  if (storageType !== 'localstorage') {
    ...
    try {
      ...
      if (adminConfig) {
        ...
        // 站长
        const ownerUser = staticConfig.USERNAME;
        if (ownerUser) {
        ...
        adminConfig = {
          SiteConfig: {
            SiteName: staticConfig.SITE_NAME || 'MoonTV',
...
export async function getConfig(): Promise<AdminConfig> {
  const staticConfig = getStaticConfig();
  const storageType = staticConfig.STORAGE_TYPE;
  if (staticConfig.IS_DOCKER || storageType === 'localstorage') {
    await initConfig();
    return cachedConfig;
  }
  ...
  if (adminConfig) {
    ...
    // 合并静态配置
    const staticConfig = getStaticConfig();
    adminConfig.SiteConfig.SiteName = staticConfig.SITE_NAME;
    adminConfig.SiteConfig.Announcement = staticConfig.ANNOUNCEMENT;
    adminConfig.UserConfig.AllowRegister = staticConfig.ENABLE_REGISTER;
    adminConfig.SiteConfig.ImageProxy = staticConfig.IMAGE_PROXY;
    adminConfig.SiteConfig.DoubanProxy = staticConfig.DOUBAN_PROXY;
    adminConfig.SiteConfig.DisableYellowFilter = staticConfig.DISABLE_YELLOW_FILTER;
