...
async function generateAuthCookie(
  ...
  if (username && config.PASSWORD) {
    ...
    // 使用密码作为密钥对用户名进行签名
    const signature = await generateSignature(username, config.PASSWORD);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (STORAGE_TYPE === 'localstorage') {
      ...
      const { password } = body;
      console.log('Extracted password:', JSON.stringify(password));
      ...
      // 调试日志
      console.log('输入的密码:', JSON.stringify(password));
      console.log('配置的密码:', JSON.stringify(envPassword));
      console.log('密码比较结果:', password === envPassword);
